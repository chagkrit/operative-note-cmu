...
function App() {
  ...
  const allNotes = React.useMemo(() => {
    ...
    driveNotes.forEach(dn => {
      if (!dn.hn && !dn.name && !dn.date) return; // Skip completely empty notes for deduplication
      const key = `${dn.hn || ""}_${dn.date || ""}_${dn.name || ""}`.toLowerCase();
      driveMap.set(key, dn);
    });

    const filteredLocal = notes.filter(ln => {
      if (!ln.hn && !ln.name && !ln.date) return true; // Keep local empty drafts
      const key = `${ln.hn || ""}_${ln.date || ""}_${ln.name || ""}`.toLowerCase();
      const dn = driveMap.get(key);
      if (!dn) return true; // Not on Drive, keep local

      const localTime = ln.updatedAt ? new Date(ln.updatedAt).getTime() : 0;
      const driveTime = dn.updatedAt ? new Date(dn.updatedAt).getTime() : 0;
      return localTime > driveTime; // Keep local only if it is strictly newer
    });

    const localKeys = new Set(filteredLocal.map(ln => 
      `${ln.hn || ""}_${ln.date || ""}_${ln.name || ""}`.toLowerCase()
    ));

    const filteredDrive = driveNotes.filter(dn => {
      const key = `${dn.hn || ""}_${dn.date || ""}_${dn.name || ""}`.toLowerCase();
      return !localKeys.has(key);
    });

    return [...filteredLocal, ...filteredDrive].sort((a, b) => (b.date || "").localeCompare(a.date || ""));
  }, [notes, driveNotes]);
